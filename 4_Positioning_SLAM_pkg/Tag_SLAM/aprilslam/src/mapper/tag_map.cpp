...
namespace aprilslam
{
    ...
    void TagMap::AddTag(const Apriltag &tag, const geometry_msgs::Pose &pose)
    {
        // Tag pose in world
        Apriltag tag_w = tag;
        UpdateTag(&tag_w, pose); // 索引观测到的tag的先验信息(世界坐标系下):pose、四个角点、center
    ...
    void TagMap::AddFirstTag(const Apriltag &tag_c)
    {
        ...
        // tags_prior_info_在类初始化已经完成了，内容是所有tag的先验信息，yaml文件导入的
        if (tags_prior_info_.find(tag_c.id) != tags_prior_info_.end())
        {   //输出的第一个tag的id
            ROS_INFO("\033[1;32m---->This is prior information of first tag, tag id is %d \033[0m", tag_c.id);
            //pose是检测到的第一个tag所对应的tags_prior_info_中的信息: translation和rotation
            pose = tags_prior_info_.find(tag_c.id)->second;
            AddTag(tag_c, pose);
        }
        else
        {
            ROS_WARN("There is no prior information about first tag: %d", tag_c.id);
            ROS_WARN("Tag map initialization failed. Try again.");
        }
    }

    void TagMap::UpdateTagsw (const std::vector<Apriltag> &tags_c) {
        for (const Apriltag tag_temp: tags_c) {
            if (tags_w_map_.find(tag_temp.id) == tags_w_map_.end()) {
                geometry_msgs::Pose pose;
                // tags_prior_info_在类初始化已经完成了，内容是所有tag的先验信息，yaml文件导入的
                if (tags_prior_info_.find(tag_temp.id) != tags_prior_info_.end()) {
                    pose = tags_prior_info_.find(tag_temp.id)->second;
                    AddTag(tag_temp, pose);
                    ROS_INFO("New Tag %d has been add to tags_w", tag_temp.id);
                }
            }
        }
    }
    ...
    bool TagMap::EstimatePose (const std::vector<Apriltag> &tags_c, const cv::Matx33d &K, const cv::Mat_<double> &D, geometry_msgs::Pose *pose)
    {
        ...
        for (const Apriltag &tag_c : tags_c)
        {
            ...
            auto it = FindById(tag_c.id, tags_w()); // 返回tags_w_中tag_w_.id=id的tag_w_元素:当前检测到的最好的标签对应的世界坐标系下的信息
            if (it != tags_w().cend())
            {
        ...
        cv::Mat c_T_w(c_t_w);   // 还是t矩阵
        cv::Mat w_R_c(c_R_w.t()); //R转置矩阵
        cv::Mat w_T_c = -w_R_c * c_T_w; // P_w=RP_c+t ==> R^T*P_w-R^T*t=P_c
        ...
        for (size_t ip = 0; ip < reproj_img_pts_pnp.size(); ip++)
        {
            ...
            reproj_error_pnp += cv::norm(origin_pt - reproj_pt);    //求解范数，即原始点和根据所求的Rt算出来的像素点之间距离
...
//        SetOrientation(&pose->orientation, w_Q_c);
    ...
    void TagMap::UpdateTagsPriorInfo(const std::map<int, geometry_msgs::Pose> tags_prior_info)
    {
        ...
        for (auto iter_tag = iter_begin; iter_tag != iter_end; iter_tag++)
        {
            ...
            // yaml里面的tag的先验信息(id、pose)+family、size信息
    ...
    {
//        std::vector<Apriltag>::iterator  it;
