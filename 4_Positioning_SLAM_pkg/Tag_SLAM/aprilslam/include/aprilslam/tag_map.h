...
namespace aprilslam
{
    ...
    public:
        ...
        void UpdateTagsw (const std::vector<Apriltag> &tags_c);

    private:
        void AddTag(const Apriltag &tag, const geometry_msgs::Pose &pose);
        
        /// @brief data structure for optimized landmarks position. 每个tag对应到世界坐标系下的信息:pose,corners
        std::vector<aprilslam::Apriltag> tags_w_;

        /// @brief data structure for prior info.  /.yaml里面的tag的先验信息id、pose、family、size信息
        std::vector<aprilslam::Apriltag> tags_w_prior_;

        /// @brief data structure for optimized landmarks position. 格式<id, tags_w_> ，每个tag对应到世界坐标系下的信息，./yaml数据
