...
namespace aprilslam
{
    ...
    private:
        ...
        ///@brief All apriltags id in SLAM framework. 已经加入地图中(检测到的)的tagc的id
        std::set<int> all_ids_;

        ///@brief Key: Apriltags id  Value: Pose in camera frame 。已经加入地图中(检测到的)的tagc
        ...
        ///@brief Key: Apriltags id  Value: Prior id、pose in /.yaml
