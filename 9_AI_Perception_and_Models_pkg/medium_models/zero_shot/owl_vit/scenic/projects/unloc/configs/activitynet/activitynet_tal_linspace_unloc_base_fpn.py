...
r"""Configs for finetuning the UnLoc model on ActivityNet TAL.
...
def get_config():
  ...
  config.dataset_configs.class_name_csv = '/path/to/labels.csv'
  config.dataset_configs.prompt_csv = '/path/to/prompts.csv'
  config.dataset_configs.num_prompts = 1
  config.dataset_configs.tokenizer_config = ml_collections.ConfigDict({
      'max_num_tokens': 16,
  ...
  config.lr_configs.decay_events = [18 * steps_per_epoch]
  ...
  config.layer_prefix_to_base_lrs = {
      'temporal_localization_head': 0.1,
  ...
  # Setting it to True will improve results but slower.
