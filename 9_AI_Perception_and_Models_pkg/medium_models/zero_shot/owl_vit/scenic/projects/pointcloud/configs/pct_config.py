...
def get_config(runlocal=''):
  ...
  config.experiment_name = 'pct-modelnet40'

  # `name` argument of tensorflow_datasets.builder()
  config.dataset = 'modelnet40_classification.1.0.0'
  config.dataset_name = 'modelnet40'
  config.data_dtype_str = 'float32'

  # Dataset config
  config.dataset_configs = ml_collections.ConfigDict()
  config.dataset_configs.dataset = 'modelnet40'
  config.dataset_configs.num_classes = NUM_CLASSES
  config.dataset_configs.train_split = 'train'
  config.dataset_configs.val_split = 'validation'
  config.dataset_configs.prefetch_to_device = 2
  config.dataset_configs.onehot_labels = True
