...
def get_config():
  ...
  config.experiment_name = 'densevoc_disjoint_pretraining'
  ...
  config.dataset_configs.temporal_stride = 4
  config.model.flatten_video_input = True
  t, res = 8, 384
  config.dataset_configs.max_frames_train = t
  config.dataset_configs.max_frames_test = t
  config.dataset_configs.input_shape = (-1, t, res, res, 3)
  config.dataset_configs.multi_dataset_training = True
  config.dataset_configs.dataset_format = (
      'imagedetection', 'imagedensecap-nodet', 'videocap', 'imagedetection-augment')
  config.dataset_configs.dataset_sample_weights = (1., 1., 1., 1.,)
  config.dataset_configs.max_frames_train = t
  config.dataset_configs.max_frames_test = 1
  config.dataset_configs.train_data_path = (
      'coco/2017',
      common.VG_TRAIN_PATH,
      common.SMIT_TRAIN_PATH,
      'coco/2017',
  )
  ...
  config.dataset_configs.num_train_examples = (
      common.NUM_EXAMPLES_SMIT_TRAIN + common.NUM_VG_TRAIN_EXAMPLES + common.NUM_EXAMPLES_COCO_TRAIN + common.NUM_EXAMPLES_COCO_TRAIN)
  config.dataset_configs.prefetch_to_device = 2
  config.dataset_configs.shuffle_buffer_size = 1000
  config.dataset_configs.max_video_captions = 1
  config.dataset_configs.max_boxes = 64
  config.dataset_configs.max_text_tokens = 40
  config.dataset_configs.scale_range = (0.1, 2.0)
  config.dataset_configs.crop_size = res
  ...
  config.model.num_text_proposals = 32
  config.model.roi_post_nms_num_detections = 16
  config.model.roi_samples_per_image = 128

  config.model.with_tracking = True
  config.model.tracking_loss_weight = 1.0

  config.model.caption_with_track = False
  config.model.roi_append_gt_boxes = True  # False
  config.model.trunc_track_score = 0.0
  config.model.asso_windows = -1

  config.model.hard_tracking = False
  config.model.frame_fuse_fn = 'concat'

  config.model.use_roi_box_in_training = False

  config.model.with_global_video_caption = True
  config.model.num_frames = config.dataset_configs.max_frames_train
  config.model.skip_global_caption_test = True

  config.model.use_loss_masks = True
