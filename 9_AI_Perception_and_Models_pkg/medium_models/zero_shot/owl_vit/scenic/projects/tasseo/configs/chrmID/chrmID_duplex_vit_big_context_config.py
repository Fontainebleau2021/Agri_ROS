...
def get_config(runlocal=''):
  ...
  config.experiment_name = 'chrmID-duplex-vit'
  # Dataset.
  ...
  num_encoders = 3  # i.e., [input, context, fused].
  # Input encoder
  config.model.patches.input_size = [CHRM_PATCH, CHRM_PATCH]
  ...
  config.model.encoder.input.num_layers = NUM_LAYERS[VERSION] // num_encoders
  ...
  # Context encoder
  config.model.patches.context_size = [CONTEXT_PATCH, CONTEXT_PATCH]
  ...
  config.model.encoder.context.num_layers = NUM_LAYERS[VERSION] // num_encoders
  ...
  config.model.encoder.fused.num_layers = NUM_LAYERS[VERSION] // num_encoders
