...
"""Data generators for the chromosome ID task."""
...
def preprocess(features, label_key):
  ...
  class_names = tf.convert_to_tensor([b'chrm_%d' % i for i in range(1, 23)] +
                                     [b'chrm_X', b'chrm_Y'])

  # Parse the input `tf.train.Example` proto using the dictionary above.
  chrm = tf.reshape(
      tf.io.decode_raw(features['chrm_img'], tf.float32), (299, 299, 1))
  meta = tf.reshape(
      tf.io.decode_raw(features['meta_img'], tf.float32), (289, 289, 1))
  labels = labels == class_names  # Creates one-hot label.

  return {
      'inputs': (
          chrm[22:-21, 22:-21],  # Clip to 256x256 to make divisible by 4.
          meta,
      ),
      'label': labels
  }
...
@datasets.add_dataset('chrmID_metaphase_context')
def get_dataset(*,
                batch_size,
                eval_batch_size,
                num_shards,
  ...
  meta_data = {
      ...
      'chrm_input_shape': input_shape,
      'metaphase_input_shape': [-1, 289, 289, 1],
      'num_train_examples': 409007,  # Number of train examples across all hosts
      'num_eval_examples': 45134,  # Number of test examples across all hosts
