...
def optimizer_with_layerwise_decay(
  ...
  This implementation is forked from av_mae:
  https://github.com/google-research/scenic/blob/main/scenic/projects/av_mae/
  optimizer_utils.py
...
def create_optimizer_and_lr_fn(params, config):
  """Returns an optimizer."""
  lr_fn = lr_schedules.get_learning_rate_fn(config)
  if config.optimizer.get('layerwise_decay', 0.0) > 0:
    tx = optimizer_with_layerwise_decay(
        config, params=params)
  elif config.optimizer.get('backbone_multiplier', -1.) >= 0.0:
    tx = optimizer_with_backbone_multiplier(
        config, params=params)
  else:
    config = config.unlock()
    for k in ['layerwise_decay', 'num_layers',
              'decay_layer_prefix', 'decay_stem_layers']:
      if k in config.optimizer:
        del config.optimizer[k]
    config = config.lock()
    tx = optimizer_lib.get_optimizer(config.optimizer, lr_fn, params=params)
  return tx, lr_fn
