...
r"""Default configs for Regularized UViT on ImageNet2012.
...
def get_config(runlocal=''):
  """Returns the UViT experiment configuration for ImageNet."""
  runlocal = bool(runlocal)
  config = ml_collections.ConfigDict()
  config.experiment_name = 'imagenet-uvit'
  # Dataset.
  ...
  config.model.parameter_sharing = False  # True or False
  # ACT Config
  # Commented out all config.model.ac_config to disable ACT
  config.model.ac_config = ml_collections.ConfigDict()
  config.model.ac_config.act_max_steps = 12
  config.model.ac_config.act_epsilon = 0.01
  config.model.ac_config.act_type = 'basic'  # 'random', 'accumulated', 'basic'
  config.model.ac_config.act_level = 'per_example'  # 'per_example', 'per_token'
  config.model.ac_config.act_halting_bias_init = -1.0
  config.model.ac_config.act_loss_weight = 0.01
  # Training.
  config.trainer_name = 'ut_trainer'
