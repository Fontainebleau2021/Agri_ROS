...
LossFn = Callable[[jnp.ndarray, Any, Batch, Optional[jnp.ndarray]], float]
...
def train_step(
  ...
  def training_loss_fn(params):
    ...
    (logits, auxiliary_outputs), new_model_state = flax_model.apply(
        ...
        debug=debug)
    loss = loss_fn(logits, auxiliary_outputs, batch, variables['params'])
  ...
  if config.model.get('ac_config', None):
    # Logging the ponder loss, n_updates (N_t) and remainders (R_t)
    ponder_times, remainders = auxiliary_outputs
    ...
    # The jnp.mean here does not consider the masking and padding.
    # It is fine for logging.
    if jnp.ndim(p_t) == 1:
      n_t_loss = jax.lax.pmean(ponder_times, axis_name='batch')
      r_t_loss = jax.lax.pmean(remainders, axis_name='batch')
      p_t_loss = jax.lax.pmean(p_t, axis_name='batch')
    else:
      n_t_loss = jax.lax.pmean(jnp.mean(ponder_times), axis_name='batch')
      r_t_loss = jax.lax.pmean(jnp.mean(remainders), axis_name='batch')
      p_t_loss = jax.lax.pmean(jnp.mean(p_t), axis_name='batch')

    training_logs['N_updates N(t)'] = n_t_loss
    training_logs['Remainder R(t)'] = r_t_loss
    training_logs['Ponder loss P_t'] = p_t_loss
