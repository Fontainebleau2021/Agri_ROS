...
r"""Default configs for BERT finetuning on QQP.
...
def get_config(variant=VARIANT, init_from=INIT_FROM):
  ...
  config.dataset_configs.task = 'qqp'
  ...
  # Training data size 363,849 examples, 3 epochs.
  config.num_training_steps = 34110
  config.log_summary_steps = 1000
  config.log_eval_steps = 5685
  # Eval data size is 40,430 examples.
  config.steps_per_eval = 1264
  config.rng_seed = 42
  ...
  config.lr_configs.warmup_steps = 3411
  config.lr_configs.base_learning_rate = 3e-05

  # Evaluation:
  config.global_metrics = ['f1']

  # Logging.
  config.write_summary = True
  config.xprof = True  # Profile using xprof.
  config.checkpoint = True  # Do checkpointing.
  config.checkpoint_steps = 3000
