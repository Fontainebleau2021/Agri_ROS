...
r"""Default configs for BERT finetuning on SST-2.
...
def get_config(variant=VARIANT, init_from=INIT_FROM):
  ...
  config.dataset_configs.task = 'sst2'
  ...
  # Training data size 67,349 examples, 3 epochs.
  config.num_training_steps = 6312
  config.log_summary_steps = 1000
  config.log_eval_steps = 1052
  # Eval data size is 872 examples.
  config.steps_per_eval = 28
  config.rng_seed = 42
  ...
  config.lr_configs.warmup_steps = 361
