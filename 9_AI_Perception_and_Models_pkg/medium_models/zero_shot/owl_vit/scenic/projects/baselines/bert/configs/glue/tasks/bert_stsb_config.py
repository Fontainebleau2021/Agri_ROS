...
r"""Default configs for BERT finetuning on STS-B.
...
def get_config(variant=VARIANT, init_from=INIT_FROM):
  ...
  config.dataset_configs.task = 'stsb'
  ...
  config.model_name = 'bert_regression'
  ...
  config.model.head.type = 'regression'
  ...
  # Training data size 5749 examples, 3 epochs.
  config.num_training_steps = 537
  config.log_summary_steps = 1000
  config.log_eval_steps = 89
  # Eval data size is 1500 examples.
  config.steps_per_eval = 47
  config.rng_seed = 42
  ...
  config.lr_configs.warmup_steps = 53
  config.lr_configs.base_learning_rate = 3e-05

  # Evaluation:
  config.global_metrics = ['pearson_corrcoef']
