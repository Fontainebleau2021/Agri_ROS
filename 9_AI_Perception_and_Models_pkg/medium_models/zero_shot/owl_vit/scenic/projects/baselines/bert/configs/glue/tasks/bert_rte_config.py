...
r"""Default configs for BERT finetuning on RTE.
...
def get_config(variant=VARIANT, init_from=INIT_FROM):
  ...
  # Training data size 2490 examples, 3 epochs.
  config.num_training_steps = 231
  config.log_summary_steps = 1000
  config.log_eval_steps = 38
  # Eval data size is 277 examples.
  config.steps_per_eval = 9
  config.rng_seed = 42
  ...
  config.lr_configs.warmup_steps = 23
