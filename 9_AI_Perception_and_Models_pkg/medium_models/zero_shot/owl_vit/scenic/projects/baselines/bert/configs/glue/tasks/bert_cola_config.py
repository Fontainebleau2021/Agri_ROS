...
r"""Default configs for BERT finetuning on COLA.
...
def get_config(variant=VARIANT, init_from=INIT_FROM):
  ...
  config.dataset_configs.task = 'cola'
  ...
  # Training data size 8551 examples, 3 epochs.
  config.num_training_steps = 801
  config.log_summary_steps = 1000
  config.log_eval_steps = 133
  # Eval data size is 1043 examples.
  config.steps_per_eval = 33
  config.rng_seed = 42
  ...
  config.lr_configs.warmup_steps = 80
  config.lr_configs.base_learning_rate = 3e-05

  # Evaluation
  config.global_metrics = ['matthews_corrcoef']
