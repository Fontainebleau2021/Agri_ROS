...
r"""Default configs for BERT finetuning on MRPC.
...
def get_config(variant=VARIANT, init_from=INIT_FROM):
  ...
  config.dataset_configs.task = 'mrpc'
  ...
  # Training data size 3668 examples, 3 epochs.
  config.num_training_steps = 342
  config.log_summary_steps = 1000
  config.log_eval_steps = 57
  # Eval data size is 408 examples.
  config.steps_per_eval = 13
  config.rng_seed = 42
  ...
  config.lr_configs.warmup_steps = 34
