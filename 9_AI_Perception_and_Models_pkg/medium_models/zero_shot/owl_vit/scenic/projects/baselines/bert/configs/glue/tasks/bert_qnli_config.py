...
r"""Default configs for BERT finetuning on QNLI.
...
def get_config(variant=VARIANT, init_from=INIT_FROM):
  ...
  config.dataset_configs.task = 'qnli'
  ...
  # Training data size 104,743 examples, 3 epochs.
  config.num_training_steps = 9819
  config.log_summary_steps = 1000
  config.log_eval_steps = 1636
  # Eval data size is 5463 examples.
  config.steps_per_eval = 171
  config.rng_seed = 42
  ...
  config.lr_configs.warmup_steps = 981
