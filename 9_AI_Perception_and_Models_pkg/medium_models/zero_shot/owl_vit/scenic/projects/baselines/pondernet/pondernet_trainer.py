...
def train_step(
  ...
  # Create rng for the Ponder sampling
  rng, ponder_rng = jax.random.split(rng)

  # Bind the rng to the host/device we are on.
  dropout_rng = train_utils.bind_rng_to_host_device(
      rng, axis_name='batch', bind_to='device')
  ponder_rng = train_utils.bind_rng_to_host_device(
      ponder_rng, axis_name='batch', bind_to='device')
  ...
  _, _, n_updates = auxiliary_outputs
  n_t = jax.lax.pmean(n_updates, axis_name='batch')
  training_logs['n_updates N(t)'] = n_t
...
def eval_step(
  ...
  # Create rng for the Ponder sampling
  _, ponder_rng = jax.random.split(train_state.rng)
  ponder_rng = train_utils.bind_rng_to_host_device(
      ponder_rng, axis_name='batch', bind_to='device')
...
def train(
  ...
  # Initialize model.
  # We need to pass two rng to init the model here.
