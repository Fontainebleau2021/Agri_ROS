...
def beam_search(inputs: jnp.ndarray,
  ...
  def beam_search_loop_body_fn(state: BeamState) -> BeamState:
    ...
    # --> [batch, per_node_beam*beams, length]
    topk_seq = gather_beams(state.live_seqs, topk_beam_indices, batch_size,
                            beam_size, beams_to_keep)
    # Update sequences for the per_node_beam*K top-k new sequences.
    # --> [batch, per_node_beam*beams, length]
    ...
    # Determine the top k beam indices (from top per_node_beam*k beams)
    # from log probs. --> [batch, beams]
    _, new_topk_indices = lax.top_k(new_log_probs, k=beam_size)
    new_topk_indices = jnp.flip(new_topk_indices, axis=1)
    # Gather the top k beams (from top per_node_beam*k beams).
    # --> [batch, beams, length], [batch, beams]
    top_alive_seq, top_alive_log_probs = gather_beams(
        [topk_seq, new_log_probs], new_topk_indices, batch_size,
        per_node_beam_size * beam_size, beam_size)
    ...
    # --> [batch, (1+per_node_beam)*beams, length]
    finished_seqs = jnp.concatenate([state.finished_seqs, topk_seq], axis=1)
    finished_scores = jnp.concatenate(  # --> [batch, (1+per_node_beam)*beams]
        [state.finished_scores, new_scores], axis=1)
    finished_flags = jnp.concatenate(  # --> [batch, (1+per_node_beam)*beams]
...
def autoregressive_predict(
  ...
  Args:
    ...
    outputs: dict with keys:
        'visual_features': (batch_size, num_tokens, hidden_size) or
          (batch_size, num_caps_per_image, num_tokens, hidden_size)
  ...
  visual_feature_dim_expanded = False
  if visual_features.ndim == 3:
    visual_feature_dim_expanded = True
  ...
  begin_tokens = begin_tokens.reshape(
      (total_batch_size,) + begin_tokens.shape[2:])
  if 'context_tokens' in outputs and outputs['context_tokens'] is not None:
  ...
  if beam_size > 1:
    ...
    visual_features = jnp.broadcast_to(
        ...
        (total_batch_size, beam_size,) + visual_features.shape[1:]).reshape(
            (total_batch_size * beam_size,) + visual_features.shape[1:]
        )
    if context_tokens is not None:
      context_tokens = jnp.broadcast_to(
          context_tokens[:, None],
          (total_batch_size, beam_size, context_tokens.shape[1])).reshape(
              total_batch_size * beam_size, context_tokens.shape[1])
  ...
  if method == 'beam':
    ...
    kwargs['per_node_beam_size'] = per_node_beam_size
    kwargs['alpha'] = brevity_penalty_alpha
  text_tokens, log_probs = decode_fn(
      ...
      **kwargs)
  outputs['beam_text_tokens'] = text_tokens.reshape(
      total_batch_size, beam_size, flax_model.max_caption_length)
  outputs['beam_log_probs'] = log_probs.reshape(total_batch_size, beam_size)
  # output of beam search scores are in increasing order.
  outputs['text_tokens'] = outputs['beam_text_tokens'][:, -1]
  outputs['log_probs'] = outputs['beam_log_probs'][:, -1]
  if not visual_feature_dim_expanded:
    outputs['text_tokens'] = outputs['text_tokens'].reshape(
        (batch_size, num_caps_per_image,) + outputs['text_tokens'].shape[1:])
    outputs['log_probs'] = outputs['log_probs'].reshape(
        (batch_size, num_caps_per_image))
    outputs['beam_text_tokens'] = outputs['beam_text_tokens'].reshape(
        (batch_size, num_caps_per_image,)
        + outputs['beam_text_tokens'].shape[1:])
    outputs['beam_log_probs'] = outputs['beam_log_probs'].reshape(
        (batch_size, num_caps_per_image,) + outputs['beam_log_probs'].shape[1:])
  return outputs
