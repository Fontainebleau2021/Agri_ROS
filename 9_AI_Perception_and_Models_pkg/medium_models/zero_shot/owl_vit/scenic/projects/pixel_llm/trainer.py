...
"""Training script for PixelLLM models."""
...
from scenic.projects.pixel_llm import partition_utils
from scenic.projects.pixel_llm import train_utils as pixel_llm_train_utils
...
def train_and_evaluate(
  ...
  if hasattr(model, 'prepare_input_spec'):
    input_spec = model.prepare_input_spec(dataset.meta_data)
  else:
    input_spec = [(
  ...
  model_kwargs = {}
  if 'force_init' in config:
    model_kwargs['force_init'] = config.force_init
  ...
  train_state, params_axes = pixel_llm_train_utils.pop_axes_names(
      train_state, 'params_axes')
  train_state = checkpoints.restore_checkpoint(workdir, train_state)
  train_state = pixel_llm_train_utils.re_add_axis_names(
      train_state, params_axes, 'params_axes')

  start_step = int(train_state.global_step)
  if start_step == 0:
    train_state, start_step = pixel_llm_train_utils.load_weights(
        train_state, config)
  ...
  for step in range(start_step + 1, total_steps + 1):
    ...
    eval_first_step = config.get('eval_first_step', True) and (
        step == start_step + 1
    )
    do_eval = not config.get('not_eval', False)
    if (
        (step % log_eval_steps == 0) or (step == total_steps) or eval_first_step
    ) and do_eval:
      ...
      with report_progress.timed('eval'):
        ...
        eval_summary = evaluate.inference_on_all_datasets(
            model,
            train_state,
            dataset,
            writer,
    ...
    if (step % checkpoint_steps == 0 and step > 0) or (step == total_steps):
