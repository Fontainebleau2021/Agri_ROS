...
def _build_pipeline(
  ...
  ds_already_batched = False  # flag indicates whether ds is batched or not
  if merge_sources:
    batch_before_merge = config.get('batch_before_merge', False)
    logging.warning('batch_before_merge: %s', batch_before_merge)
