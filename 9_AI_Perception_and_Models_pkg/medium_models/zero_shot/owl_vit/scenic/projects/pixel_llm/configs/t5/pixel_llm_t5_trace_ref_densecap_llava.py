...
r"""Default configs for joint training on COCO caption, Localized Narratives, Referring Expression Localization, Dense Object Captioning, Visual Instruct Tuning with T5.
...
def get_coco_cap_train_source(tokenizer_path, max_text_tokens, max_context_tokens, num_captions_per_sample, crop_size, weight=1.0):
  ...
  preproc_spec_train = (
      ...
      f'|pad_detection_annotations({num_captions_per_sample})'
...
def get_vg_train_source(tokenizer_path, max_text_tokens, max_context_tokens, max_boxes, crop_size, *, weight=1.0):
  ...
  preproc_spec_train = (
      ...
      f"|add_prompt_tokens('{tokenizer_path}', {max_boxes}, {prompt}, {max_context_tokens}, {append_eos})"
      f"|add_prompt_boxes"
      f"|add_task_mask(['detection', 'caption'])"
  )
...
def get_llava_cap_train_source(tokenizer_path, max_text_tokens, max_context_tokens, num_captions_per_sample, crop_size, dataset_name='llava_v1_5_mix665k', weight=1.0):
  """Returns the LLaVA train source."""
  ...
  preproc_spec_train = (
      "parse_llava"
      f"|decode_coco_caption_annotations('{tokenizer_path}', {num_captions_per_sample}, {max_text_tokens}, {max_context_tokens}, {append_eos}, context_prefix='Question: ', context_suffix='Answer: ')"
      f"|random_ratio_resize({min_scale}, {max_scale}, {crop_size})"
      f"|fixed_size_crop({crop_size})"
      f"|init_padding_mask"
      f"|pad_images({crop_size}, {crop_size})"
      f"|pad_caption_annotations({num_captions_per_sample})"
      f'|pad_detection_annotations({num_captions_per_sample})'
      f'|pad_loco_annotations({num_captions_per_sample}, 2)'
      f"|add_prompt_boxes"
      f"|add_task_mask(['caption'])"
  )
  context_features = {
      ...
      'image/id': {'feature_type': 'VarLen', 'dtype': 'string'},
      'conversations/human': {'feature_type': 'VarLen', 'dtype': 'string'},
      'conversations/agent': {'feature_type': 'VarLen', 'dtype': 'string'},
  }
  tfrecord_meta = {
      'llava_v1_5_mix665k': {
          'tfrecords': common.LLAVA_V1_5_MIX665K.path,
          'size': common.LLAVA_V1_5_MIX665K.size,
      },
      'sharegpt4v_mix665k_cap23k_coco-ap9k_lcs3k_sam9k_div2k': {
          'tfrecords': common.SHAREGPT4V_MIX665K.path,
          'size': common.SHAREGPT4V_MIX665K.size,
      },
  }
...
def get_config():
  ...
  config.experiment_name = 'pixel_llm_t5_trace_ref_densecap_llava'
  ...
  config.dataset_configs.train.sources = [
      get_coco_cap_train_source(tokenizer_path, max_text_tokens, max_context_tokens, max_boxes, crop_size, weight=3.0),
      get_ln_trace_cap_train_source(tokenizer_path, max_text_tokens, max_context_tokens, max_boxes, crop_size, dataset_name='coco', weight=1.0),
      ...
      get_vg_train_source(tokenizer_path, max_text_tokens, max_context_tokens, max_boxes, crop_size, weight=5.0),
      get_llava_cap_train_source(tokenizer_path, max_text_tokens, max_context_tokens, max_boxes, crop_size, dataset_name='sharegpt4v_mix665k_cap23k_coco-ap9k_lcs3k_sam9k_div2k', weight=5.0),
  ]
