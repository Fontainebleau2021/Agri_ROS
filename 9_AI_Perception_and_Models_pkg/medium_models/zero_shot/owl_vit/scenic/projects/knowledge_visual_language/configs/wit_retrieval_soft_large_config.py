...
def get_config() -> ml_collections.ConfigDict:
  ...
  # config.optimizer_configs.epsilon = 1e-6
  # config.optimizer_configs.weight_decay = 1e-2
  # config.optimizer_configs.mu_dtype = 'bfloat16'

  config.optimizer = 'adafactor'
  batch_size = 12 * 2 * 128
  config.optimizer_configs = ml_collections.ConfigDict()
  config.optimizer_configs.momentum = 0.9
  config.dtype_momentum = 'bfloat16'
  ...
  config.model.retrieval_ratio = 1 / 3
