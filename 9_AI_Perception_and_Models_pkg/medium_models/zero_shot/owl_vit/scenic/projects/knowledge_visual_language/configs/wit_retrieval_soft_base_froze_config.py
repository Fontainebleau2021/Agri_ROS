...
def get_config() -> ml_collections.ConfigDict:
  ...
  config.not_frozen_patterns = [('value_perceiver/.*', 2),
                                ('text_encoder/.*', 1),
                                ('shared_token_embedder/.*', 0.5),
                                ('query_head/.*', 1.5), ('out_decoder/.*', 1),
                                ('key_head/.*', 1.5), ('img_encoder/.*', 1),
                                ('head_out/.*', 1.5), ('fusion_encoder/.*', 1),
                                ('att_transform/.*', 1)]
  ...
  config.model.vit_num_frozen_layers = -1
  ...
  config.lr_configs.base_learning_rate = 6e-4
