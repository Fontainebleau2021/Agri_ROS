...
def train_step(
  ...
  def training_loss_fn(params):
    ...
    loss, all_logits, new_model_state = 0, {}, None
    for dim in config.matvit_dims:
      logits, new_model_state = flax_model.apply(
          ...
          matvit_mask_dims=[dim] * config.model.num_layers,
          debug=debug,
      )
      all_logits[dim] = logits
      loss += loss_fn(logits, batch, variables['params'])

    return loss, (new_model_state, all_logits)

  compute_gradient_fn = jax.value_and_grad(training_loss_fn, has_aux=True)

  (
      train_cost,
      (new_model_state, all_logits),
  ...
  all_metrics = {}
  for dim, logits in all_logits.items():
    metrics = metrics_fn(logits, batch)
    all_metrics[f'pred_{dim}d@1'] = metrics['prec@1']
    all_metrics[f'loss_{dim}d'] = metrics['loss']
  ...
  return new_train_state, all_metrics, training_logs
