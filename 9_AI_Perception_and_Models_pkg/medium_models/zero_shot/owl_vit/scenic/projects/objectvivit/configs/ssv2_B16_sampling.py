...
r"""ObjectViViT finetuning experiments on Something-Something v2.
...
def get_config(runlocal=''):
  """Return the config of ObjectViViT experiment on Something-Something v2."""

  runlocal = bool(runlocal)

  config = ml_collections.ConfigDict()
  config.experiment_name = 'ssv2_B16_sampling'
  ...
  ############################# Object configs ################################
  config.dataset_configs.object_configs = ml_collections.ConfigDict()
  config.dataset_configs.object_configs.with_boxes = True
  config.dataset_configs.object_configs.bbox_key = 'orvit'  # gt
  config.dataset_configs.object_configs.keep_full_frames = 0
  config.dataset_configs.object_configs.mask_radius = 0.7
  config.dataset_configs.object_configs.concat_mask = True
  config.dataset_configs.object_configs.tracked_objects = True
  num_boxes = 4
  config.dataset_configs.object_configs.bbox_num = num_boxes
  ##############################################################################

  ####################### Attach Object configs ################################
  config.attach_configs = ml_collections.ConfigDict()
  config.attach_configs.object_block_idx = (1, 6, 10)
  config.attach_configs.token_score_from_dataloader = True
  config.attach_configs.drop_pixel_tokens = True
  config.attach_configs.add_context_tokens = int(8 * 14 * 14 * 0.1)
  config.attach_configs.num_total_attach_tokens = int(8 * 14 * 14 * 0.3)
  ##############################################################################
