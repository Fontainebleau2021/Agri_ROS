...
"""Default config for launching a GER-ALD finetuning."""
...
def get_config():
  ...
  config.model.label_smooth = 0.1
  config.model.dropout_prob = 0.1
  config.model.decode_beam_size = 4
  config.weights = path_to_pretrained_gerald_model_weights
  ...
  config.num_training_steps = 30000
  ...
  config.lr_configs.base_learning_rate = 3e-7
  config.early_stopping = 8500
