...
def get_config():
  ...
  config.experiment_name = 'git_youcook2_streaming_input_output'

  # Dataset.
  config.dataset_name = 'flexio'
  ...
  def get_preproc_spec_train(num_frames, num_dense_outputs, early_segments_as_context, normalize_early_timestamps, num_bins, context_mask_ratio, only_use_augmented_context, dynamic_location):
    preproc_spec_train = (
        f"decode_and_subsample_densecap_video({num_frames}, False, zero_pad_frames=False)"
  ...
  config.checkpoint_steps = 100
  config.eval_first_step = False
  config.checkpoint_max_to_keep = 20
