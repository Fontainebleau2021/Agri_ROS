...
#include "lslidar_to_velodyne/convert.h"
...
{
    // nh_local_.param<string>("rslidar_topic_", rslidar_topic_, "/c16/lslidar_point_cloud");
    // cout<<rslidar_topic_<<"\n";
    // nh_local_.param<string>("velodyne_topic", velodyne_topic_, "/velodyne_points");
    nh_local_.getParam("lslidar_topic_", rslidar_topic_);
    cout<<"Listening: "<<rslidar_topic_<<"\n";
    nh_local_.getParam("velodyne_topic", velodyne_topic_);
}
...
void Convert::callback(const sensor_msgs::PointCloud2ConstPtr& cloud)
{
    ...
    rs_pc.height = 16;
    pcl::PointCloud<velodyne_pcl::PointXYZIRT> velo_pc;
#ifdef DEBUG
    pcl::PointCloud<velodyne_pcl::PointXYZIR> velo_pc_per_ring[16];
#endif // DEBUG
    for(int i = 0; i < 16; i++)
    {
        for(unsigned int j = 0; j < rs_pc.size() / 16; j++)
        {
            velodyne_pcl::PointXYZIRT velo_point;
            ...
            {
                velo_point.ring =7+i/2;
            }
           // velo_point.time= i*3125;
...
#ifdef DEBUG
    pcl::PointCloud<velodyne_pcl::PointXYZIR>  velo_pc_filter;
    for(int i = 0; i < 16; i++)
    {
        pcl::PointCloud<velodyne_pcl::PointXYZIR> velo_pc_per_ring_filter;
        for(int j = 1; j < velo_pc_per_ring[i].size()-1; j++)
        {
            velodyne_pcl::PointXYZIR& last_point = velo_pc_per_ring[i][j-1];
            double range_before = sqrt(last_point.z*last_point.z + 
            last_point.x*last_point.x + last_point.y*last_point.y);

            velodyne_pcl::PointXYZIR& cur_point = velo_pc_per_ring[i][j];
            double range_cur = sqrt(cur_point.z*cur_point.z + 
            cur_point.x*cur_point.x + cur_point.y*cur_point.y);

            velodyne_pcl::PointXYZIR& next_point = velo_pc_per_ring[i][j+1];
...
{
    ros::init(argc, argv, "ls_to_velodyne");
    ros::NodeHandle nh, nh_local("ls_to_velodyne");
