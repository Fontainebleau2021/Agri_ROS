...
void publish_points(T &new_pc, const sensor_msgs::PointCloud2 &old_msg) {
    ...
    pc_new_msg.header.frame_id = output_frame;
...
int main(int argc, char **argv) {
    ...
    ros::NodeHandle nh("~");
    std::string input_topics_str, output_topics_str;
    nh.getParam("input_topic", input_topics_str);
    nh.getParam("output_topic", output_topics_str);
    nh.getParam("output_type", output_type);
    nh.getParam("output_frame", output_frame);

    if (output_type.empty()) {
        ROS_ERROR("Please specify output pointcloud type (XYZI, XYZIR, XYZIRT)!!!");
        exit(1);
    }

    if (input_topics_str.empty() || output_topics_str.empty()) {
        ROS_ERROR("Please specify input and output topics!!!");
        exit(1);
    }

    if (output_type == "XYZI") {
        subRobosensePC = nh.subscribe(input_topics_str, 1, rsHandler_XYZI);
    } else if (output_type == "XYZIRT") {
        subRobosensePC = nh.subscribe(input_topics_str, 1, rsHandler_XYZIRT);
    } else {
        ROS_ERROR("Unsupported input pointcloud type. Currently only support XYZI and XYZIRT.");
        exit(1);
    }

    pubRobosensePC = nh.advertise<sensor_msgs::PointCloud2>(output_topics_str, 1);

    ROS_INFO("Listening to %s ......", input_topics_str.c_str());
    ros::spin();
    return 0;
}
